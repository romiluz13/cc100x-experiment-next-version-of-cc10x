...
Score = Probability x Impact. Address risks with score > 8 first.
...
# Do NOT auto-commit from this skill.
# Keep changes staged/unstaged and let user/lead decide when to commit.
```
...
```
TaskCreate({
  subject: "CC100X Execute Plan: {feature}",
...
TaskCreate({
  subject: "CC100X Phase 1: {phase_title}",
...
TaskCreate({
  subject: "CC100X Phase 2: {phase_title}",
  description: "**Plan:** docs/plans/YYYY-MM-DD-{feature}-plan.md\n**Section:** Phase 2\n**Exit Criteria:** {demonstrable_milestone}\n\n{phase_details}",
  activeForm: "Working on {phase_title}"
})
...
Update `.claude/cc100x/progress.md` with task *subjects* (and optionally task IDs for the current session).
...
- CC100X Execute Plan: {feature} (blocked by: -)
- CC100X Phase 1: {title} (blocked by: -)
- CC100X Phase 2: {title} (blocked by: CC100X Phase 1: {title})
")
...
- Bad: "Phase 1: Foundation" - Vague, when is it done?
- Good: "Phase 1: User can authenticate" - Demonstrable, testable
...
**1. Agent Teams-Driven (this session)** - Builder teammate implements with live reviewer, then verification pipeline
