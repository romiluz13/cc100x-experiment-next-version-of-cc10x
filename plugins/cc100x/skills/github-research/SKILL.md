...
**This skill is ONLY for external research.** Local codebase search is handled by other CC100x tools (Grep/Glob/Read).
...
### Phase 3: Summarize for CC100x Memory
...
**Two-Phase Execution (via cc100x-lead):**

The lead executes research BEFORE invoking agents. This is NOT a hint - it's a prerequisite.

```
1. Lead detects github-research trigger (explicit request OR knowledge gap)
2. Lead executes octocode tools directly (this skill's guidance)
3. Lead passes research results to agent in prompt
```

**Research results passed to:**
- `planner` agent - For informed planning decisions
- `investigator` agent - For external error patterns

**Never used during:**
- `security-reviewer` / `performance-reviewer` / `quality-reviewer` - Review focuses on code itself
- `hunter` - Speed is priority
- `verifier` - Verification, not research
...
- IMMEDIATELY proceed to Step 2 (do not invoke agents, do not pass go)
